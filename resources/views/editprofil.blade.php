            ...
            <div class="w-full max-w-screen-sm mx-auto items-center p-4 bg-brand-desk border border-brand-desk rounded-lg shadow sm:p-6 md:p-8">
                <form class="space-y-6" action="/editprofil/{{ Auth::user()->id }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="relative w-14 h-14 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600">
                        @if (Auth::user()->image)
                        <img src="{{ asset('storage/' . $user->image) }}" class="img-preview absolute w-14 h-14 text-gray-400 blur-sm hover:blur-none">    
                        @else
                        <img class="img-preview absolute w-14 h-14 text-gray-400 blur-sm hover:blur-none">
                        {{-- <svg class="absolute w-16 h-16 text-gray-400 -left-1 blur-sm hover:blur-none" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path></svg> --}}
